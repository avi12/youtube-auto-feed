...
const USER_DATA_SRC = join(process.env.APPDATA!, "Opera Software/Opera Stable");
const USER_DATA_DEST = resolve(import.meta.dirname, "../Opera Data WXT");
const OPERA_PATH = join(process.env.LOCALAPPDATA!, "Programs/Opera/opera.exe");
const START_URL = "https://www.youtube.com/feed/subscriptions";
const PROCESS_NAME = "opera.exe";

function killExistingBrowser() {
  try {
    execSync(
      `powershell -NoProfile -Command "Get-WmiObject Win32_Process -Filter \\"name='${PROCESS_NAME}'\\" | Where-Object { $_.CommandLine -like '*Opera Data WXT*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -EA SilentlyContinue }"`,
      { stdio: "ignore" }
    );
  } catch {}
}
...
function setupProfile() {
  ...
  execSync(
    process.platform === "win32"
      ? `xcopy "${USER_DATA_SRC}" "${USER_DATA_DEST}" /E /I /Q /Y`
      : `cp -r "${USER_DATA_SRC}" "${USER_DATA_DEST}"`,
    { stdio: "ignore" }
  );
}
...
function build() {
  return new Promise<boolean>(resolve => {
    const child = spawn("bun", ["run", "wxt", "build"], { stdio: "pipe", shell: true });
    let output = "";
