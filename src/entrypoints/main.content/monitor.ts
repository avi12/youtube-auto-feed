...
export function createSubscriptionMonitor() {
  ...
  function handleBrowseResponse(event: Event) {
    ...
    if (!isInnerTubeBrowseResponse(event.detail)) return;
  ...
  async function fetchFreshVideos() {
    if (!isOnSubscriptionsPage() || !isDomReady) return false;
  ...
  async function fetchAndApplyMetadataUpdates() {
    if (!isOnSubscriptionsPage() || !isDomReady || isApplyingChanges) return;
  ...
  function handlePageFocus() {
    if (document.hidden || !isOnSubscriptionsPage() || !isDomReady) return;
